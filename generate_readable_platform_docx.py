"""
Generate a properly spaced, readable Word document from Platform Capability Statement
"""
...
def process_markdown_to_docx(md_file_path, output_path):
    """Process markdown file and create Word document with proper spacing"""
    ...
    # Set default style with proper spacing
    ...
    while i < len(lines):
        ...
        # Skip horizontal rules but add spacing
        if line.strip() == '---':
            add_spacing_paragraph(doc)
            i += 1
            continue
        ...
        # Empty lines
            ...
            if i > 0 and i < len(lines) - 1:
                ...
                if lines[i-1].strip() and lines[i+1].strip() and not lines[i+1].startswith('#'):
                    add_spacing_paragraph(doc)
    ...
    print(f"  - Font: 11pt Calibri with 1.15 line spacing")
    print(f"  - Proper spacing between sections")
